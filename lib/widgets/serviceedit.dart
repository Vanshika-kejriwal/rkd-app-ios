...
class ServiceEdit extends StatefulWidget {
  ...
  ServiceEdit({super.key, this.assignExisting = false, this.leadinfo});
    ...
    if (response.statusCode == 200) {
      for (var c in body) {
        product.add(LeadProduct(
            company: c["CNAME"], product: c["LEADFORP"], mc: c["MC"]));
      }
    }
    setState(() {
      _company = product.toList();
    });
  }
